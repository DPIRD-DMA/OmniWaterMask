...
### Changed
- Road lines are now buffered in the raster's own projected CRS instead of by way of EPSG:3857. **This changes the mask.** EPSG:3857's scale factor is 1/cos(latitude), so `buffer(distance=5)` laid down about 4.24 m of ground at Perth's latitude, and a different amount at every other latitude, which gave the same code different real-world road widths in Kununurra and Hobart. Roads are now a true 5 m, about 18% wider, and set 1.7% more negative-target pixels on the area measured (7,470,943 to 7,598,563). Negative targets suppress water, so slightly more of the scene is masked than before. A raster in a geographic CRS still takes the EPSG:3857 route, since it has no metre units of its own. The reprojection pair was also 2.1 GB of the 2.7 GB this step spent, so peak memory over it drops from 2.67 GB to 1.26 GB.
- The detection threshold search evaluates the whole curve in one pass over a histogram, rather than walking it one candidate at a time with a bounded Brent minimiser. It therefore finds the grid's global optimum instead of whichever local one the search happened to reach, so IoU is equal or better everywhere measured, and **output differs slightly**: 20,766 of 120,560,400 pixels on the tile measured (0.017%), water IoU 0.99977, water fraction 74.107% to 74.121%. Together with rewriting the per-cluster ratio loop as one grouped reduction, which is bit-identical, `multi_scale_optimisation` on a Sentinel-2 tile (10980x10980, 74% water) goes from 42.0 s to 5.5-6.1 s. Both loops were dominated by device syncs of about 0.2 ms each, not by the work they guarded.
- Resampling no longer writes a second copy of the scene to disk. `resample_input` described the resampled grid as a VRT in memory instead of writing a GeoTIFF beside the output, so nothing is left behind where a 4-band Sentinel-2 tile previously left 362 MB at 20 m or 5.8 GB at 5 m, and resample-and-read on a 5490x5490 4-band scene goes from 0.62 s to 0.22 s. Pixels, CRS, transform, size and bounds are identical across both down- and up-sampling. If you relied on that intermediate file existing, it is gone.
- Peak memory is lower through the target and debug paths, with output bitwise identical in both. Target layers are combined in place rather than stacked and reduced, which also stops a `bool` sum being promoted to `int64` at eight bytes a pixel for values that never exceed three: fusing four boolean layers goes from 1380 MB to 115 MB. The debug export hands layers over as they are and promotes each one as it writes, rather than promoting and stacking all 14 up front, taking peak RSS on a 6000x6000 scene with every debug layer from 6.22 GB to 2.22 GB. Writing a band at a time needs a matching layout, so debug exports are now BAND interleaved and tiled.
- `inference_dtype` now defaults to `"auto"` instead of `torch.float32`. Under `"auto"` the inference device is measured once — a real convolution, repeated until it has run for 50 ms — and a reduced-precision dtype is used only where it beats float32 by more than 10%; otherwise float32 is kept, so the previous behaviour is what a device that gains nothing still gets. This is measured rather than read off a capability flag because a device will accept `bfloat16` and run every kernel on it while emulating the arithmetic in software, which is far slower than the float32 it was meant to beat (~100x on this machine's CPU). Candidates are screened with one small call before the real probe, so an emulating path is dropped rather than timed at full size. Where a reduced-precision path does win, the choice is `bfloat16` ahead of `float16` — it keeps float32's exponent range, and probe timings were not a reliable guide to how the whole model ranks two dtypes sharing one hardware path — with `float16` used only where `bfloat16` will not run. The result is cached per device, so a batch of scenes measures once; selection costs ~130 ms on MPS and ~380 ms on CPU. On a Perth Sentinel-2 tile this took model inference from 27.5 s at float32 to 21.1 s at bfloat16, with output identical to six decimal places. Reduced precision is not bit-identical to float32, so pass an explicit dtype (`torch.float32`, or `"bf16"` to force the drop) where an exact match to previous output matters.
- The vector cache now records a build that found no features, instead of leaving it a miss. An empty result is the expensive one to recompute - the query has to scan the whole bounding box before it can report that there is nothing in it - and it can only ever come back empty again, so every run was repeating it. The entry is a database row with no parquet beside it, which is what separates "nothing here" from "not looked up yet". A fetch that fails is not an empty result and is not recorded: both vector sources raise once their retries are exhausted rather than returning an empty frame, so a network failure cannot be written down as "no water here".
- The vector cache database is now `geodataframes_v4.db`, so entries written by earlier versions are ignored rather than migrated. Two changes make them unusable: entries now store geometry that has already been buffered in the raster's own projected CRS rather than in EPSG:3857, which changes the distance laid down on the ground by 1/cos(latitude), and the cache key now includes that CRS. Storing geometry in the raster's CRS makes an entry reusable only by a raster sharing it, and the rest of the key does not imply one - two rasters can share a WGS84 bounding box and not a CRS, and rasterization reprojects nothing, so serving the wrong entry would have produced a blank or displaced mask with no error. Existing caches are not deleted, only no longer read; `prune_orphaned_gdfs` reclaims the space.
- Cached parquet files now live in a directory carrying the cache generation, `gdfs_v4`, rather than in one `gdfs` directory shared across generations. Sharing it was what made a superseded generation's files anonymous: once its database was no longer read, nothing on disk recorded which files had belonged to it, so telling them apart from a genuine orphan meant scanning every row of the current database and then guessing about the rest. A generation is now one database and one directory, and dropping it is a delete rather than a deduction.
- `prune_stale_cache` is exported from the package, reclaiming what old generations leave behind - a bump strands a full copy of the parquets, which is most of what a cache directory weighs. It deletes generations below the current one whole, and any parquet in the current generation that no entry points at, which is now unambiguous since nothing else writes into that directory. A generation *above* the current one is left alone: it belongs to a newer install sharing the directory and is in use rather than obsolete. It is deliberately manual, since it destroys the only copy of data an older install would still read.
- `fastai` is no longer installed by default; it moved to a `legacy` extra (`pip install omniwatermask[legacy]`). Only model versions below 2 are fastai models. Version 2 is built with segmentation-models-pytorch, which `omnicloudmask` already requires unconditionally, so the default install gained nothing and lost the whole fastai tree. The spaCy version exclusion travels with it into the extra, since `fastai` hard-requires `spacy<4` and nothing else here pulls spaCy in — which is what made Python 3.14 installs fragile, spaCy 3.8.14 and 3.8.15 having published neither a cp314 wheel nor an sdist. Asking for a version below 2 without the extra raises an `ImportError` before anything is downloaded, naming this package's install options and the versions that do not need fastai. `omnicloudmask` raises its own version of this, but only once it goes to build the architecture - after the weights have been fetched, and worded in its version numbering and install commands, neither of which applies here.
- Model entries may now name their weights file directly as `.safetensors`. The downloader assumed an entry named a `.pth` and always rewrote the Hugging Face download as a torch state, so a safetensors-named entry would have had a pickle written to a `.safetensors` path and then been handed to `load_file`. One published file now backs both download sources for an entry, instead of each source needing its own copy.

### Fixed
- A vector cache entry whose parquet file had gone missing permanently stranded its key. The lookup warned and reported a miss, so the caller rebuilt and inserted a second row beside the broken one, and the next lookup found the broken row again - refetching and adding another duplicate on every run. The stale row is now deleted when its file is found to be missing, and a lookup considers every row matching the key rather than only the first, so a sibling that still has its file is used.
- `segmentation-models-pytorch` is now declared as a dependency. It is what the default model's architecture is built from, and `timm` — which nothing here imports either — was already declared for the same reason, so its absence was an oversight rather than a decision. It has been arriving transitively through `omnicloudmask`, which requires it unconditionally; that is exactly the arrangement `fastai` had until this release moved it to an extra, so the assumption that a dependency keeps requiring it is not one this project can make about its own model architecture. The floor mirrors what `omnicloudmask` imposes, since declaring less would not be installable alongside it rather than extending support.
