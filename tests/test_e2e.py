...
class TestIntegrateWaterDetectionReal:
    ...
    def test_real_inference_on_naip(self, naip_crop, loaded_models, tmp_path):
        ...
        result, layer_names, nodata_mask = integrate_water_detection_methods(
            input_bands=bands,
    ...
    def test_real_debug_output(self, naip_crop, loaded_models, tmp_path):
        ...
        result, layer_names, _ = integrate_water_detection_methods(
            input_bands=bands,
            input_path=naip_crop,
            cache_dir=cache_dir,
            inference_dtype=torch.float32,
...
class TestFullPipelineReal:
    ...
    def test_produces_output_file(self, naip_crop, tmp_path):
        ...
        with rio.open(output_paths[0]) as src:
            ...
            # No-data is stored as a GDAL mask rather than a data band
            assert src.read_masks(1).shape == (256, 256)
...
class TestPipelineConfigurations:
    ...
    def _validate_output(self, output_paths, expected_count=1):
        # Non-debug output is a single "Water predictions" band (no-data is
        # stored as a GDAL mask); debug callers pass a higher expected_count.
        assert len(output_paths) >= 1
